#!/usr/bin/env python3
"""
Conversor .FRM para PNG - FUNCIONANDO!
Offset correto: header_size + data_offsets (sem padding extra)
"""
...
def frm_to_png(frm_path, output_dir, palette):
    ...
    try:
        with open(frm_path, 'rb') as f:
            ...
            header_size = 62  # Tamanho do header no arquivo
            ...
            for direction in range(6):
                ...
                # Offset correto: header_size + data_offsets (sem padding extra!)
                actual_offset = header_size + data_offsets[direction]
                ...
                for frame_idx in range(frame_count):
                    try:
                        ...
                        # Caminho relativo ao diretório web_server (sem web_server/ no início)
                        rel_path = output_file.relative_to(BASE_DIR / "web_server")
...
def main():
    print("=" * 60)
    print("Conversor .FRM para PNG - VERSÃO FUNCIONANDO!")
    print("=" * 60)
    ...
    for i, frm_file in enumerate(frm_files, 1):
        ...
        if result and result['total_frames'] > 0:
            ...
            if i <= 5:  # Mostrar erro apenas nos primeiros
                print(f"    FALHOU")
    
    if len(failed) > 5:
        print(f"    ... e mais {len(failed) - 5} falharam")
    ...
    if len(converted) > 0:
        total_images = sum(s['total_frames'] for s in converted)
        print(f"\nSUCESSO! {total_images} imagens PNG criadas!")
