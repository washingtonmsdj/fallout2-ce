#!/usr/bin/env python3
"""Extrai sprites de personagens do Fallout 2"""
...
def main():
    print("Extraindo critters do Fallout 2...")
    
    # Critter.dat contem os personagens
    dat = DAT2("Fallout 2/critter.dat")
    dat.open()
    
    # Paleta do master.dat
    master = DAT2("Fallout 2/master.dat")
    master.open()
    pal = load_pal(master)
    master.close()
    
    out = Path("godot_project/assets/sprites/critters")
    out.mkdir(parents=True, exist_ok=True)
    
    # Listar critters
    critters = [f for f in dat.files.keys() if f.endswith('.frm')]
    print(f"Encontrados {len(critters)} sprites de critters")
    
    # Extrair alguns importantes (hero, npcs comuns)
    important = ['hapowr', 'hanpwr', 'hmjmps', 'hflthr', 'nmwarr', 'nfprim']
    
    count = 0
    for critter in critters:
        # Pegar apenas sprites de idle (aa)
        if 'aa.frm' not in critter:
            continue
    ...
    print(f"Extraidos: {count} sprites de critters")
    dat.close()
